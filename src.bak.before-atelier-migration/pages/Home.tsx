...
import { listDrafts, listSources, studioHealth } from '../lib/studio';
...
export default function Home() {
  const [stats, setStats] = useState({ sources: 0, drafts: 0, published: 0 });
  const [model, setModel] = useState<string>('—');
  const [clock, setClock] = useState<string>('');

  useEffect(() => {
    Promise.all([listDrafts().catch(() => []), listSources().catch(() => ({ sources: [] }))])
      .then(([ds, sr]) => {
        const drafts = Array.isArray(ds) ? ds : [];
        const srcCount = (sr as any)?.sources?.length || 0;
        setStats({
          sources: srcCount,
          drafts: drafts.filter((d: any) => d.status === 'draft').length,
